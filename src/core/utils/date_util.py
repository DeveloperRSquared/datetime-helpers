...
def get_nth_business_day_of_month(  # pylint: disable=invalid-name
    n: int, dt: Optional[datetime.date] = None
) -> datetime.date:
    dt = dt or datetime.date.today()
...
def datetime_to_string(  # pylint: disable=invalid-name
    dt: datetime.datetime, date_format: str = JSON_DATE_FORMAT
) -> str:
    return dt.strftime(date_format)


def date_to_string(  # pylint: disable=invalid-name
    dt: datetime.date, date_format: str = "%Y-%m-%d"
) -> str:  # pylint: disable=invalid-name
    return dt.strftime(date_format)


def datetime_from_string(
    text: str, datetime_format: str = JSON_DATE_FORMAT
) -> datetime.datetime:
